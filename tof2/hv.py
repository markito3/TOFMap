...
#    print(liner)
    if i == 1:
        ...
        for j in range(1, nslots + 1):
            ...
            if label[0:4] == 'TOF-':
                cable_id = cable_id + 1
                print("INSERT INTO hvCable SET id = " + str(cable_id) + ", label = \"" + label + "\", hvCardLocationId = " + str(j) + ", channel = \"" + channel + "\", pmtLocationId = (SELECT id FROM pmtLocation WHERE label = \"" + label + "\");")
